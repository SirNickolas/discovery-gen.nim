...
const
  ...
  rootTypesModule = "d_types"
  rootSerializationModule = "d_serialization"
...
proc emitMemberType(e; api; names; tyNamespace: string; memberName: string; ty: Type):
  ...
  e.emit:
    ...
    of stkI64: "long"
    of stkU64: "ulong"
    of stkDate, stkDateTime, stkDuration, stkString, stkFieldMask, stkBase64: "string"
    of stkEnum:
...
func initRootTypesCodegen(c; settings): Codegen =
  declareCodegen('#', e):
    "autogenerated":
    ...
    "module":
      e.emit &("module {settings.package}.{settings.names.rootResource}." & rootTypesModule & ";\p")
      e.endSection
...
func qualifyTypeName(
  header: TypeDeclHeader; info: TypeDeclHeaderNameInfo; tyNamespace: string;
): string =
  (if header.isHiddenType info: "P." else: tyNamespace) & info.name

proc emitEnumMemberNames(e; api; names) =
  var s = ""
  for i, en in api.enumDecls:
    let info = names.enumNameInfos[i]
    s.setLen 0
    for m in en.members:
      s.addSep ", "
      s.formatValue Quoted m.bare.name, ""
...
proc emitUrlSerializer(
  ...
  for i, m in params.members:
    if (
      ...
      api.getEnum(m.bare.ty.scalar.enumId).members.len <= 1 and
      m.bare.ty.containers.len == 0
...
func initRootSerializationCodegen(c; settings): Codegen =
  declareCodegen('#', e):
    ...
    "module":
      e.emit fmt do:
        "module {settings.package}.{settings.names.rootResource}." & rootSerializationModule & ";\p"
      e.endSection
    ...
    "imports":
      # TODO: Do not forget that 64-bit numbers serialize as strings.
      if c.api.usedTypes * stkSomeInteger != { }:
      ...
      e.emit fmt do:
        "import {settings.package}.d.serialization;\p" &
        "import {settings.package}.{settings.names.rootResource}." & rootTypesModule & ";\p"
      e.endSection
    ...
    "enumMemberNames":
      e.emit "immutable {\p"
      e.indent
      e.emitEnumMemberNames c.api, settings.names
      e.dedent
      e.emit "}\p"
      e.endSection

    "bufferUrlInitializer":
      e.emit &dd"""
      package void initForUrl(scope ref Buffer b) {{
        b.clear();
        b ~= {Quoted c.rawApi.rootUrl & c.rawApi.servicePath};
      }}
      """
      e.endSection
...
#[
func initRootPackageCodegen(c: Context): Codegen =
  declareCodegen('#', e):
    "autogenerated":
      e.emit autogeneratedComment
...
func prepareFiles(c; settings): seq[(string, GenFileSpec)] =
  ...
  var
    fileSet = @{
      "types": GenFileSpec(
        path: path & '/' & rootTypesModule & ".d",
        indent: settings.indentation,
        codegen: c.initRootTypesCodegen settings,
      ),
      "serialization": GenFileSpec(
        path: path & '/' & rootSerializationModule & ".d",
        indent: settings.indentation,
        codegen: c.initRootSerializationCodegen settings,
      ),
    }
