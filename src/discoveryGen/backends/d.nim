...
type
  ResourceContext = object
    ...
    fileSet: seq[(string, GenFileSpec)]
    path, packagePrefix: string
...
func initMethodsCodegen(c; settings; packagePrefix: string; methods: openArray[Method]): Codegen =
  declareCodegen('#', e):
    "autogenerated":
      e.emit autogeneratedComment
    ...
    "module":
      e.emit fmt do:
        "module {settings.package}.{settings.names.apiName}.{packagePrefix}" & methodsModule & ";\p"
      e.endSection

    "methods":
      # for m in methods:
      #   e.emit &"// {m.name} {m.pathFragments}"
      e.endSection
...
proc prepareSubpackages(ctx: var ResourceContext; resources: openArray[Resource]) =
  let pathCheckpoint = ctx.path.len
  let packageCheckpoint = ctx.packagePrefix.len
  for res in resources:
    let name = res.name # TODO: Rename.
    ctx.path &= '/'
    ctx.path &= name
    ctx.packagePrefix &= name
    ctx.packagePrefix &= '.'
    ctx.fileSet.add (ctx.packagePrefix & "methods", GenFileSpec(
      path: ctx.path & '/' & methodsModule & ".d",
      indent: ctx.settings.indentation,
      codegen: ctx.c.initMethodsCodegen(ctx.settings, ctx.packagePrefix, res.methods),
    ))
    ctx.prepareSubpackages res.children

    ctx.path.setLen pathCheckpoint
    ctx.packagePrefix.setLen packageCheckpoint

func prepareFiles(c; settings): seq[(string, GenFileSpec)] =
  var ctx = block:
    let srcPath = settings.pathPattern % settings.names.apiName
    let package = settings.package.replace('.', '/')
    ResourceContext(c: c, settings: settings, path: &"{srcPath}/{package}/{settings.names.apiName}")

  ctx.fileSet.add ("types", GenFileSpec(
    path: ctx.path & '/' & typesModule & ".d",
    indent: settings.indentation,
    codegen: c.initTypesCodegen settings,
  ))
  # TODO: Emit top-level REST methods.
  ctx.prepareSubpackages c.api.resources
  ctx.fileSet
