      ...
      of "object":
        ...
        without itemSchema =? member.additionalProperties:
          result.scalar = ScalarType(
            flags: {stfHasDefault},
            kind: stkStruct,
            structId: c.registerAnonStructType c.analyzeStructBody member.properties,
          )
          break
...
proc reorderStructMembers(members: var openArray[StructMember]) =
  members.sort byTypeAndName
    #[
      ...
      In anonymous structs, total ordering also helps to deduplicate them more aggressively.
    ]#
